...
class CheckoutForm extends Component
{
    ...
    public function chargeCard()
    {
        ...
        // extract month and year from $this-expiery_date
        $expiry_month = substr($this->expiry_date, 0, 2);
        $expiry_year = substr($this->expiry_date, 3, 4);

        if (env('APP_ENV') == 'local') {
            $gateway = "http://uberpayment.test/api/charge";
        } else {
            $gateway = "https://uber-pay.com/api/charge";
        }

        $response = Http::post($gateway, [
            'amount' => $this->total,
        ...
        if ($response->successful()) {
            ...
            if ($errorData) {
                info("Parsed JSON: " . json_encode($errorData));
            } else {
